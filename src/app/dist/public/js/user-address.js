...
function postAddress(dataToSend) {
    ...
    fetch(url, {
        ...
        .then((response) => {
        if (!response.ok) {
            throw new Error(`Gửi dữ liệu không thành công. Mã trạng thái: ${response.status}`);
        }
        return response.json();
    })
        .then((data) => {
...
function handleFormAddress(form) {
    ...
    if (hasError) {
        if (firstEmptyInput !== null) {
            ...
            firstEmptyInput.oninput = () => {
                ...
                firstEmptyInput.parentElement.querySelector(".form__message").textContent = "";
            };
        }
        return false;
    }
