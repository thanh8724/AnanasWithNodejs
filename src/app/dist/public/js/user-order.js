...
function renderOrders(listOrders) {
    ...
    listOrders.forEach(order => {
        if (order.idAccount == idUser) {
            if (order.productOrderInfo.status == "waitForConfirmation")
                statusOrder = "Chờ xác nhận!";
            if (order.productOrderInfo.status == "delivering")
                statusOrder = "Đang giao!";
            if (order.productOrderInfo.status == "delivered")
                statusOrder = "Đã giao!";
            orderHtml += `<tr>
                            <td>#${order.id}</td>
...
export {};
//# sourceMappingURL=user-order.js.map