...
function getPageUpdate() {
    const forms = document.querySelectorAll("#formUpdateOrder");
    forms.forEach((form) => {
...
getPageUpdate();
updateOrder();
function renderPageUpdate(data, listProduct) {
    const infoReceivers = document.querySelectorAll(".infoReceiver");
    const containerSelectUpdateBill = document.querySelector(".selectUpdateBill");
    const buttonUpdateOrder = document.querySelector("#buttonUpdateOrder");
    const boxNameProductPerBill = document.querySelector('.boxNameProductPerBill');
    let methodShiping = "";
    ...
    if (containerSelectUpdateBill && data.status === "Đã giao") {
        ...
        if (buttonUpdateOrder)
            buttonUpdateOrder.style.display = 'none';
    }
    else {
        if (containerSelectUpdateBill) {
            ...
            const listOtion = containerSelectUpdateBill.querySelectorAll('option');
            if (listOtion) {
                if (data.status == "Chờ xác nhận")
                    listOtion[0].selected = true;
                if (data.status == "Đang giao")
                    listOtion[1].selected = true;
            }
        }
        if (buttonUpdateOrder)
            buttonUpdateOrder.style.display = 'block';
    }
    // gán giá trị cho input hidden để có thể lấy giá trị
    const statusOldOrder = document.querySelectorAll("#status-id__OldOrder");
    if (statusOldOrder) {
