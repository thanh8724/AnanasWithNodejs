...
export const getDashboardPage = async (req, res, next) => {
    ...
    const customer = dataAccount.accounts.filter((account) => account.roleAccount == 'user');
    let totalAmouts = 0;
    fileOrder.bills.forEach((bill) => {
...
export const getAccountsPage = async (req, res, next) => {
...
export const getCreateCategoryPage = async (req, res, next) => {
    const categories = await readFileJson('./src/json/dataHomePage.json');
    const { nameCategory } = req.body;
    const imageUrl = `/uploads/${req.file?.filename}`;
    const checkCategory = categories.categories.some((item) => item.nameCategory == nameCategory);
...
export const handleDeleteCategory = async (req, res, next) => {
    ...
    const category = data.categories.find((item) => item.id == idCategory);
...
export const renderPageUpdate = async (req, res, next) => {
    const data = await readFileJson('./src/json/dataHomePage.json');
    const { idCategory } = req.body;
    const category = data.categories.find((item) => item.id == idCategory);
...
export const handleUpdateCategory = async (req, res, next) => {
    ...
    let imageUrl = ``;
    const otherCategories = data.categories.filter((item) => item.id != idCategory);
    const checkCategory = otherCategories.some((item) => item.nameCategory == nameCategoryUp);
...
export const getCreateProductPage = async (req, res, next) => {
    const data = await readFileJson('./src/json/dataProductsPage.json');
    ...
    if (typeof req.files == 'object' && Object.keys(req.files).length !== 0) {
        const avatarFile = req.files['inputAvatarproduct'][0];
        const imagesFiles = req.files['inputImagesProduct'];
        const imageProduct = `/uploads/${avatarFile.filename}`;
        const arrayImagesDetail = imagesFiles.map(file => `/uploads/${file.filename}`);
        const checkProductExists = listProduct.some((product) => product.nameProduct == nameProduct);
...
export const handleDeleteProduct = async (req, res, next) => {
    ...
    const product = data.products.find((e) => e.id == idProduct);
    const listUrlImageDetail = data.imagesDetail.filter((e) => e.idProduct == idProduct);
    unLinkImage(product.imageProduct);
    listUrlImageDetail.forEach((element) => {
...
export const renderPageUpdateProduct = async (req, res, next) => {
    const data = await readFileJson('./src/json/dataProductsPage.json');
    const { idProduct } = req.body;
...
export const handleUpdateProduct = async (req, res, next) => {
    ...
    let imageUrl = '';
    const otherProducts = data.products.filter((item) => item.id != idProductUpdate);
    const checkProductExists = otherProducts.some((item) => item.nameProduct == nameProductUp);
    if (req.file?.filename === undefined) {
        console.log("not change image");
...
// orders
export const openUpdateOrder = async (req, res, next) => {
    const { idOrder } = req.body;
    await createDataBillsJsonFile();
    const data = await readFileJson('./src/json/dataBills.json');
    const bill = data.bills.find((item) => item.id == idOrder);
    const listProduct = data.productbill.filter((item) => item.idBill == idOrder);
...
export const handleUpdateOrder = async (req, res, next) => {
    ...
    if (newStatus === oldStatus)
        return res.status(400).json({ title: "Thất bại!", message: "Trạng thái đơn hàng chưa thay đổi!" });
    if (newStatus === "Chờ xác nhận" && oldStatus === "Đang giao")
        return res.status(400).json({ title: "Thất bại!", message: "Có lỗi trong quá trình cập nhật trạng thái!" });
    else {
...
// account
export const handleCreateAccount = async (req, res, next) => {
    ...
    let roleAccount = '';
    if (checkboxRoleUser == undefined)
        roleAccount = 'admin';
    if (checkboxRoleAdmin == undefined)
        roleAccount = 'user';
    if (checkboxRoleUser == undefined && checkboxRoleAdmin == undefined)
        roleAccount = 'user';
    const dataAccount = {
...
export const handleDeleteAccount = async (req, res, next) => {
    ...
    const account = data.accounts.find((e) => e.id == idAccount);
